...
from base.menu_manager import MenuManager
...
class BaseController(ABC):
    ...
    def __init__(self, data: dict, data_desc: str):
        self.__data = data  # Initializing the private instance variable for Movies
        self.__menu_items = self.__get_menu_dictionary(data_desc)
        self.menu_manager = MenuManager(self.__menu_items)
        self.data_manager = DataManager(self.__data)
    ...
    # region DELETE
    @abstractmethod
    def delete_item_in_data(self) -> bool:
    ...
    def __get_menu_dictionary(self, data_desc) -> dict:
        return {
            ...
            4: {"method": self.delete_item_in_data, "desc": f"Delete {data_desc}".title()},
            ...
            12: {"method": self.show_data_histogram, "desc": f"Show {data_desc}s Histogram".title()},
            13: {"method": self.save_data, "desc": f"Save".title()}
        }
